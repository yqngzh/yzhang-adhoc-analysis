--------   Level2 boost first page TIRE result (run 2)
-- query data
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_query_data` as (
...
-- listing data
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_listing_data` as (
    with fb_data as (
        select 
            `key` as listing_id, 
...
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_raw` as (
    with tire_output_all as (
        ...
        and tireRequestContext.tireTestv2Id = "h1920Mjw33xJwgJpBZb5"
        and request.limit != 0
    ),
    selected_pages as (
    ...
    from tire_output
    left join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_query_data` qdata
    on tire_output.query = qdata.query
    left join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_listing_data` ldata
...
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_full` as (
    select r.*, p.qtd_distrib, p.listing_taxo_distrib, p.distrib_distance
    from `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_raw` r
    join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_processed` p
    on r.uuid = p.uuid
...
FROM `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_full` 
...
--------   Impression match
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_match` as (
    with fb_data as (
...
with tmp as (
    ...
    from `etsy-sr-etl-prod.yzhang.qtd_level2_tire_v2_match`
