...
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2full_tire_raw` as (
    with tire_output_all as (
        ...
        and tireRequestContext.tireTestv2Id = "czAQ0eEy4NfZCyNd7WkR"
        and request.limit != 0
    ),
    selected_pages as (
...
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2full_tire_full` as (
    select r.*, p.qtd_distrib, p.listing_taxo_distrib, p.distrib_distance
    from `etsy-sr-etl-prod.yzhang.qtd_level2full_tire_raw` r
    join `etsy-sr-etl-prod.yzhang.qtd_level2full_tire_processed` p
    on r.uuid = p.uuid
...
FROM `etsy-sr-etl-prod.yzhang.qtd_level2full_tire_full` 
...
--------   Impression match
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2full_tire_match` as (
    with fb_data as (
...
with tmp as (
    ...
    from `etsy-sr-etl-prod.yzhang.qtd_level2full_tire_match`
