--------   Level2 distribution force TIRE result
-- query data
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_query_data` as (
    with query_fb_data as (
        ...
        from `etsy-ml-systems-prod.feature_bank_v2.query_feature_bank_2023-11-17`
...
-- listing data
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_listing_data` as (
    with fb_data as (
        select 
            `key` as listing_id, 
        ...
        from `etsy-ml-systems-prod.feature_bank_v2.listing_feature_bank_2023-11-29`
    )
    select 
        listing_id,
...
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_raw` as (
    with tire_output_all as (
        ...
        where DATE(queryTime) = '2023-11-29'
        and tire_results.request.query != ""
        and tireRequestContext.tireTestv2Id = "1ikeducYd96MdTndKXs7"
        and request.limit != 0
    ),
    selected_pages as (
    ...
    from tire_output
    left join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_query_data` qdata
    on tire_output.query = qdata.query
    left join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_listing_data` ldata
...
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_full` as (
    select r.*, p.qtd_distrib, p.listing_taxo_distrib, p.distrib_distance
    from `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_raw` r
    join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_processed` p
    on r.uuid = p.uuid
...
FROM `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_full` 
...
--------   Impression match
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_match` as (
    with fb_data as (
...
with tmp as (
    ...
    from `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_match`
...
--------   Sanity check
create or replace table `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_sanity` as (
    with tire_output_all as (
        select 
            response.mmxRequestUUID as uuid,
            request.query,
    ...
    select 
        ...
        qdata.query_bin, qdata.query_intent, qdata.ppaths, qdata.pcounts,
        ldata.listing_taxo_level2
    from tire_output_all as tire_output
    left join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_query_data` qdata
    on tire_output.query = qdata.query
    left join `etsy-sr-etl-prod.yzhang.qtd_level2_tire_dforce_listing_data` ldata
    on tire_output.listing_id = ldata.listing_id
