...
from utils import utils, graphite, beam_fn
from utils.eval import eval_utils


config = load_preprocess_config("/home/yzhang/development/neural_ranking/second_pass/pipeline/configs/preprocess/sr_web.yaml|prod")
eval_config = load_eval_config("/home/yzhang/development/neural_ranking/second_pass/pipeline/configs/eval/sr.yaml|sem_rel")
end_date = "2024-06-28"
eval_end_date = "2024-06-29"
saved_model_path = f"gs://training-prod-search-data-jtzn/neural_ranking/second_pass/models/nrv2-sem-rel-pw-web/train/{end_date}/main/export/single_device_model"


train_num_of_days = config.max_num_of_days
input_path_to_prefix_dict = {
    "gs://etldata-prod-search-ranking-data-hkwv8r/feature_logging_training_data_parquet/query_pipeline_web_organic/tight_purchase": "search.metrics.ranking.tensorflow_ranking_model"
}
input_path_to_sampling_dict = {
    "gs://etldata-prod-search-ranking-data-hkwv8r/feature_logging_training_data_parquet/query_pipeline_web_organic/tight_purchase": "full"
}
dated_input_file_list = utils.get_dated_input_file_list(
    end_date=eval_end_date,
    num_of_days=[1],
    input_base_path_list=["gs://etldata-prod-search-ranking-data-hkwv8r/feature_logging_training_data_parquet/query_pipeline_web_organic/tight_purchase"],
    mode="full",
)
...
columns = list(
    set(
        ...
        + eval_utils.FEATURES_FOR_OVERLAP_TAG
    )
)
ranker_name = "TFRanking"
...
with beam.Pipeline() as pipeline:
    (
        ...
        | "Print" >> beam.Map(lambda x: print(x[1]["attributions"]))
    )

# (path, data with type dict)
# dict key: feature names
# dict values: list of feature values in 1 request (48 listing for web, 34 for mweb, 28 for boe)
