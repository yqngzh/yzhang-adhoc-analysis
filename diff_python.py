...
with DAG(
    dag_id="semantic_relevance_baseline_per_experiment",
    catchup=True,
    max_active_runs=4,
    max_active_tasks=4,
    schedule="0 16 * * *",
    ...
    sampling_table = (
        "etsy-data-warehouse-prod.search.sem_rel_hydrated_daily_requests_per_experiment"
    )

    inference_result_table_prefix = ""
    inference_output_pairs_table_name = (
        "etsy-data-warehouse-prod.search.sem_rel_query_listing_metrics_per_experiment"
    )
    inference_output_requests_table_name = (
        "etsy-data-warehouse-prod.search.sem_rel_requests_metrics_per_experiment"
    )
    ...
    sample_search_requests_for_inference = BigQueryExecuteQueryOperator(
        ...
        sql="SELECT * FROM `etsy-data-warehouse-prod.search.sem_rel_get_requests_per_every_exp`('{{macros.etsy.hyphenated_days_ago(1,ds)}}')",
    ...
    job_name_prefix = "sem-rel-experiment-dashboard"
    ...
    for model_args in model_args_dicts:
        ...
        dataflow_options = [
            ...
            image,
            "--experiment",
            "max_workflow_runtime_walltime_seconds=43200",
        ]
